/**
 * Sections 1-11: activation as a migration, and the lane rules it installs.
 *
 * dryRun previews the tag->lane mapping and a digest while writing nothing; a
 * commit echoing that digest applies it; a stale digest is refused. Then the
 * rules the migration leaves behind — which lanes a human may write, that the
 * lane structure itself is locked, owner-only activation, re-activation
 * clearing stranded tasks to the Inbox, and deactivation restoring the plain
 * tag list.
 *
 * The sections share no JavaScript state: each re-reads the board over HTTP, so
 * what carries between them is the worker's own persisted D1/KV state.
 */
import { CONTRIB, LANES, OWNER, repoOfVia, type Ctx, type Lane } from './automation-context'

export async function runCoreFlow(ctx: Ctx) {
  const { req, mcp, check, section, tasks } = ctx
