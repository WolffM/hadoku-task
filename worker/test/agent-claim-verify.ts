...
const env = { TASKS_KV: makeKV(), DB: d1, EDGE_AUTH_SECRET: EDGE_SECRET, TASK_STORAGE: 'd1' } as Record<string, unknown>
...
async function req(method: string, path: string, body?: unknown): Promise<{ status: number; json: Body | null }> {
...
function forceExpire(taskId: string) {
  ...
  d1.__raw.prepare('UPDATE task_claims SET expires_at = ? WHERE user_id = ? AND task_id = ?').run(past, OWNER.id, taskId)
}
...
async function main() {
  ...
  const pv = await req('POST', '/task/api/boards/auto/activate-automation', { lanes: LANES, schemaId: 's', schemaVersion: 1, dryRun: true })
  const digest = (pv.json as { preview?: { digest?: string } } | null)?.preview?.digest
  await req('POST', '/task/api/boards/auto/activate-automation', { lanes: LANES, schemaId: 's', schemaVersion: 1, digest })
  ...
  const results = await Promise.all(
    Array.from({ length: 5 }, (_, i) => req('POST', '/task/api/agent/claim', { board: 'auto', taskId: 't1', agentId: `agent-${i}` }))
  )
  ...
  // ---------------------------------------------------------------------
  let r = await req('POST', '/task/api/agent/claim', { board: 'auto', taskId: 't1', agentId: 'thief' })
  check('re-claim while live → 409 CLAIM_HELD', r.status === 409 && r.json?.code === 'CLAIM_HELD', `status=${r.status}`)
  ...
  check('heartbeat with the live token → 200', r.status === 200 && r.json?.ok === true, JSON.stringify(r.json))
  r = await req('POST', '/task/api/agent/heartbeat', { board: 'auto', taskId: 't1', token: 'not-the-token' })
  check('heartbeat with a stale token → 409 LEASE_LOST', r.status === 409 && r.json?.code === 'LEASE_LOST', `status=${r.status} ${JSON.stringify(r.json)}`)

  // ---------------------------------------------------------------------
  section('5. set-lane is the agent path (§5.2)')
  // ---------------------------------------------------------------------
  r = await req('POST', '/task/api/agent/set-lane', { board: 'auto', taskId: 't1', token, lane: 'working' })
  ...
  r = await req('POST', '/task/api/agent/set-lane', { board: 'auto', taskId: 't1', token, lane: 'bogus' })
  check('set-lane to a non-lane → 422 LANE_UNKNOWN', r.status === 422 && r.json?.code === 'LANE_UNKNOWN', `status=${r.status}`)
  r = await req('POST', '/task/api/agent/set-lane', { board: 'auto', taskId: 't1', token: 'stale', lane: 'review' })
  check('set-lane without the claim → 409 LEASE_LOST', r.status === 409 && r.json?.code === 'LEASE_LOST', `status=${r.status}`)
  ...
  check('human into an agent lane → 403 LANE_NOT_EDITABLE', r.status === 403 && r.json?.code === 'LANE_NOT_EDITABLE', `status=${r.status}`)
  ...
  r = await req('POST', '/task/api/agent/release', { board: 'auto', taskId: 't1', token, lane: 'review', ifCurrentLane: 'needs-work' })
  check('release with a stale ifCurrentLane → 409 LANE_CHANGED', r.status === 409 && r.json?.code === 'LANE_CHANGED', `status=${r.status} ${JSON.stringify(r.json)}`)
  ...
  r = await req('POST', '/task/api/agent/release', { board: 'auto', taskId: 't1', token, lane: 'review', notes: 'the plan', outcome: 'ok', ifCurrentLane: 'working' })
  check('release → 200 released', r.status === 200 && r.json?.released === true, JSON.stringify(r.json))
  check('task moved to the release lane', (await tag('t1')) === 'review')
  const t1 = (await req('GET', '/task/api/tasks?boardId=auto')).json?.tasks?.find(t => t.id === 't1')
  ...
  // ---------------------------------------------------------------------
  r = await req('POST', '/task/api/agent/release', { board: 'auto', taskId: 't1', token, lane: 'done' })
  check('replayed release → 200 no-op (released:false)', r.status === 200 && r.json?.released === false, JSON.stringify(r.json))
  ...
  // ---------------------------------------------------------------------
  r = await req('POST', '/task/api/agent/claim', { board: 'auto', taskId: 't1', agentId: 'agent-B' })
  ...
  // ---------------------------------------------------------------------
  forceExpire('t1')
  r = await req('POST', '/task/api/agent/claim', { board: 'auto', taskId: 't1', agentId: 'agent-C' })
  ...
  check('the stolen-from token → 409 LEASE_LOST', r.status === 409 && r.json?.code === 'LEASE_LOST', `status=${r.status}`)
  // Release with the new token to free the task.
  await req('POST', '/task/api/agent/release', { board: 'auto', taskId: 't1', token: token3, lane: 'working' })
  ...
  check('a released claim recorded ended_by=release', r.json?.history?.some(h => h.endedBy === 'release') === true)
  ...
  check('poll since cursor sees the new task', feed2.json?.changes?.some(c => c.id === 't2') === true, JSON.stringify(feed2.json?.changes))
  check('poll since cursor does NOT replay old tasks', !feed2.json?.changes?.some(c => c.id === 't1'), JSON.stringify(feed2.json?.changes))

  // ---------------------------------------------------------------------
  section('14. MCP drives the protocol + forwards the structured code (§4.3)')
  // ---------------------------------------------------------------------
  await req('POST', '/task/api', { id: 't3', title: 'mcp task', boardId: 'auto', tag: 'needs-work' })
  ...
  check('MCP double-claim → isError with CLAIM_HELD code', mc2.isError === true && mc2.structuredContent?.code === 'CLAIM_HELD', JSON.stringify(mc2))
  const mcRel = await mcp('release_claim', { taskId: 't3', board: 'auto', token: mcpToken, lane: 'review', notes: 'done via mcp' })
