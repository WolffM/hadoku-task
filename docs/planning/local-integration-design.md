...
| Area              | Decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Source of truth   | ~~Cloudflare KV~~ → **D1** as of [`agent-boards-design.md`](agent-boards-design.md) (T1). The HTTP contract and `src/domain/types.ts` are unchanged, so the plugin is unaffected; only the server-side storage engine moves.                                                                                                                                                                                                                                                                                                         |
...
| Write safety      | **L1 + L2 optimistic concurrency**: board carries a `version`; writes present `If-Match`; mismatch → `409`, client re-pulls + retries. Backward-compatible (no `If-Match` ⇒ legacy last-write-wins). ⚠️ **Superseded in part** — the "true CAS = Durable Object (deferred)" escape hatch is no longer the plan; task+board content moves to **D1**, which gives real compare-and-swap. See [`agent-boards-design.md`](agent-boards-design.md). The `version`/`If-Match` contract itself is kept, so `TaskApiClient` needs no change. |
...
4. Keep the existing in-memory per-instance `withBoardLock`. **Honest framing: this narrows lost
   ...
   > **Superseded (2026-07-21).** The escape hatch named here was a Durable Object. It isn't — the
   > decision is to move task + board content to **D1**, which is already bound (`env.DB`) and already
   > on the cold-load path, and which gives real compare-and-swap via a conditional `UPDATE` +
   > `meta.changes`. This became necessary once multiple **agents** (not just one human on two
   > devices) needed to claim tasks without both believing they won. Once content lives in D1,
   > `withBoardLock` is dead weight and gets deleted rather than left as a false comfort.
   > Full design: [`agent-boards-design.md`](agent-boards-design.md).
