...
Nothing here is built yet. This exists so TenHands can review the contract before hadoku-task
implements it — the tranche plan is in [`agent-boards-design.md`](agent-boards-design.md) §9;
activation + lane enforcement land in T5, the claim/lease runtime in T6.

What we're asking for: confirm the **mechanism** works for you (activation payload shape,
claim/heartbeat/release, error codes, `notes`), and send back the **lane set** you actually want. The
draft in `schemas/tenhands-v1.json` is a conversation starter, not a spec.
