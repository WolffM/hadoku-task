/**
 * Theme-family icons — one per theme pair in THEME_FAMILIES.
 *
 * These lived in @wolffm/task-ui-components, which is why @wolffm/themes had to
 * import from it, which is why the dependency arrow ran themes -> ui-components,
 * which is why HadokuThemeContext could not be defined next to the provider that
 * fills it. That split is what let a duplicated ui-components module produce two
 * distinct React contexts and take three apps down on 2026-08-05.
 *
 * They belong here: they are theme metadata, they are consumed by THEME_FAMILIES
 * a few lines away, and they depend on nothing but React. Moving them reverses
 * the arrow and lets the context live with its provider.
 *
 * UI chrome (SettingsIcon, TagIcon) and the fallback shapes stay in
 * task-ui-components — those belong to the components that draw them.
 */
import React from 'react'

/** Shared SVG attributes. Duplicated from task-ui-components' icon file rather
 *  than imported: this package must not depend on that one — that dependency is
 *  the whole reason the icons moved. Seventeen lines of constants is the right
 *  price for a one-way graph. */
const iconProps = {
  width: 20,
  height: 20,
  viewBox: '0 0 24 24',
