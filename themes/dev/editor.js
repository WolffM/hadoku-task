...
import {
  COLOR_VARS, CASCADE_MAP, TEXT_CONTRAST_MAP,
  THEME_FAMILIES, THEME_ICONS, THEME_ICON_MAP,
  LOCAL_SERVER_URL, WORKER_URL
} from './config.js';
...
function updateThemePreview() {
  ...
  $('previewThemeName').textContent = currentTheme
    .split('-').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ') + ' Theme';
  ...
  if (simpleMode) {
    ...
    primaryCard.className = 'preview-card-large metallic-surface metallic-surface--noise metallic-surface--highlight card-gradient-offset';
...
function updateModCount() {
  ...
  if (count === 0) {
    ...
    container.innerHTML = Object.entries(modifications).map(([varName, value]) => `
...
function exportCSS() {
  const count = Object.keys(modifications).length;
  if (count === 0) { showToast('No modifications to export'); return; }

  let css = `/* Theme modifications based on ${currentTheme} */\n`;
  css += `[data-theme='${currentTheme}-custom'] {\n`;
  Object.values(COLOR_VARS).flat().forEach(varName => {
    const value = getVarValue(varName);
...
async function saveToFile() {
  if (!localServerAvailable) { showToast('Local server not running'); return; }
  const count = Object.keys(modifications).length;
  if (count === 0) { showToast('No modifications to save'); return; }

  try {
    const response = await fetch(`${LOCAL_SERVER_URL}/save-theme`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ themeName: currentTheme, css: generateThemeCSS(currentTheme), mode: 'update' })
    });
...
function openSubmitModal() {
  const count = Object.keys(modifications).length;
  if (count === 0) { showToast('No modifications to submit'); return; }
...
async function submitTheme() {
  ...
  if (!themeName) { showSubmitStatus('error', 'Theme name is required'); return; }
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape') {
    ...
    else if (pickerOpen) { pickerOpen = false; renderThemePicker(); }
  }
});
...
window.editor = {
  openPanel, closePanel, setTheme, togglePicker, toggleSurfaceMode,
  handleColorPick, handleHexInput, handleSliderChange,
  copyCurrentVar, resetCurrentVar, resetAll, exportCSS,
  saveToFile, openSubmitModal, closeSubmitModal, submitTheme
};
